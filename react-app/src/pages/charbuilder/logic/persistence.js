...
export function makeSheetPayload(character, data) {
  ...
  return {
    ...
    adapterRuntime: {
      ...
      speciesResources: (installedRegistry.getSpeciesSheetResources(character.speciesName, character.speciesSource) || []).map(r => ({ ...r, maxComputed: typeof r.max === 'function' ? r.max(character.level) : r.max })),
      ...
      featResources: getSelectedFeatNames(character).flatMap((feat) => (installedRegistry.getFeatSheetResources(feat) || []).map(r => ({ ...r, maxComputed: typeof r.max === 'function' ? r.max(character.level) : r.max }))),
