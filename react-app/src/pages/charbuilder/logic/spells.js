...
import { getFinalScore, getPrimaryClassLevel, getSpellSlots, statMod } from './calculations.js';
...
export function getSpellCounts(character) {
  ...
  const cantrips = character.cls?.cantripProgression?.[level - 1] || profile.cantripKnown?.[level - 1] || 0;
  let spells = character.cls?.preparedSpellsProgression?.[level - 1] || profile.preparedSpellsProgression?.[level - 1] || 0;
