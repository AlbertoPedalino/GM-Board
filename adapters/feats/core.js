import { getFeatAdapter, getGlobalFeatAdapters } from '../registry.js';
  ...
  if (Array.isArray(pr.ability) && pr.ability.length) {
    ...
    pr.ability.forEach(function (ab) {
      ...
      Object.entries(ab).forEach(function (entry) {
        const k = entry[0];
        const v = entry[1];
        const labelMap = (typeof window.SLBL !== "undefined" && window.SLBL) || {};
...
function _featGetCustomAdapter(name, source) {
  const byFull = getFeatAdapter(name, source);
  if (typeof byFull === "function") return byFull;
  const byName = getFeatAdapter(name, "");
...
function _featRunGlobalAdapters(feat, ctx) {
  ...
  const list = getGlobalFeatAdapters();
...
export function adaptFeatRecord(rawFeat, ctx) {
...
export function adaptFeatsDataset(rawFeats, ctx) {
  return _featToArray(rawFeats).map(function (feat) {
    return adaptFeatRecord(feat, ctx);
  });
}

export function getFeatCategoryCodes(feat) {
...
export function featHasCategory(feat, category) {
...
export function getFeatMinLevel(feat) {
...
export function formatFeatPrerequisite(feat) {
  if (!feat || typeof feat !== "object") return "";
...
if (_glob) {
  _glob.adaptFeatRecord = adaptFeatRecord;
  _glob.adaptFeatsDataset = adaptFeatsDataset;
  _glob.getFeatCategoryCodes = getFeatCategoryCodes;
  _glob.featHasCategory = featHasCategory;
  _glob.getFeatMinLevel = getFeatMinLevel;
  _glob.formatFeatPrerequisite = formatFeatPrerequisite;
}
